...
namespace Chat.Tests.BLL.Validators.Tests
{
    ...
    public class ValidationTests
    {
        ...
        [TestCase("image.xls")]
        public void FileTypesValidatorTest_ThrowsException_ForInvalidType(string fileName)
        {
            //arrange
            var bytes = Encoding.UTF8.GetBytes("TestFile");
            var avatar = new FormFile(new MemoryStream(bytes), 0, bytes.Length, "Data", fileName);
            
            //act
            void act() => FileTypesValidator.IsValidFileType(avatar);
            //assert
            var ex = Assert.Throws<ChatException>(() => act());
        }
        ...
        [TestCase("image.png")]
        [TestCase("image.jfif")]
        public void FileTypesValidatorTest_ReturnsNull_ForValidTypes(string fileName)
        {
            //arrange
            var bytes = Encoding.UTF8.GetBytes("TestFile");
            var avatar = new FormFile(new MemoryStream(bytes), 0, bytes.Length, "Data", fileName);
